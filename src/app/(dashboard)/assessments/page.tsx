...
export default async function AssessmentsPage() {
  ...
  const [userSkills, assessments] = await Promise.all([
    prisma.userSkill.findMany({ where: { userId: session.user.id }, include: { skill: true }, orderBy: { confidenceScore: "desc" } }),
    prisma.assessment.findMany({ where: { userId: session.user.id }, orderBy: { createdAt: "desc" } }),
  ]);
  ...
  return (
    <div>
      <DashboardHeader title="AI Assessments" subtitle="Verify your skill level with AI-generated quizzes" userInitials={initials} username={(session.user as any).username} />
