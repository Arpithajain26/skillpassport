...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = (await aiClient.roadmap({
      current_skills: userSkills.map((s) => ({
    ...
    if (isValidUserId(userId)) {
      try {
        const existing = await prisma.learningRoadmap.findFirst({
          where: { userId, careerGoalId: careerGoal?.id },
        });
        if (existing) {
    ...
    return NextResponse.json({
      ...
      overview:
        "Structured 4-week learning path targeted at critical skill gaps.",
      weeks: [
        {
          week: 1,
          title: "TypeScript Deep Dive",
          description: "Master generics and types",
          tasks: ["Build TS project"],
          skills_targeted: ["TypeScript"],
          resources: ["TS Docs"],
        },
        {
          week: 2,
          ...
          tasks: ["Build backend module"],
          skills_targeted: ["Node.js"],
          resources: ["API Handbook"],
        },
      ],
